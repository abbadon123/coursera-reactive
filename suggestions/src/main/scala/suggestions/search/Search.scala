...
object Search {
  ...
  def wikipediaSuggestionJson(term: String): Future[List[String]] = {
    async {
      ...
      val response = await { Http(url(search + term).setFollowRedirects(true).OK(as.String)) }
  ...
  val restAdapter = new RestAdapter.Builder().setServer("http://en.wikipedia.org")
  .setDebug(true)
  .build()
...
//  mk     def wikipediaSuggestion(term: String): Future[List[String]] = wikipediaSuggestionRetrofit(term)
  def wikipediaSuggestion(term: String): Future[List[String]] = wikipediaSuggestionJson(term)

  
//  mk def wikipediaPage(term: String): Future[String] = wikipediaPageRetrofit(term)
   def wikipediaPage(term: String): Future[String] = wikipediaPageJson(term)
