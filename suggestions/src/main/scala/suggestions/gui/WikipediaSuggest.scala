...
object WikipediaSuggest extends SimpleSwingApplication with ConcreteSwingApi with ConcreteWikipediaApi {
  ...
  def top = new MainFrame {
    ...
    // TO IMPLEMENT
    val suggestions: Observable[Try[List[String]]] = searchTerms.sanitized.concatRecovered { term => wikiSuggestResponseStream(term) }

    // TO IMPLEMENT
    val suggestionSubscription: Subscription =  suggestions.observeOn(eventScheduler) subscribe {
      x => x match {
        case Success(terms) => suggestionList.listData = terms
        case Failure(e) => status.text = e.getMessage
      }
    }

    // TO IMPLEMENT
    val selections: Observable[String] = button.clicks.map { _  => suggestionList.selection.items.mkString }

    // TO IMPLEMENT
    val pages: Observable[Try[String]] = selections.sanitized.concatRecovered { selection => wikiPageResponseStream(selection) }

    // TO IMPLEMENT
    val pageSubscription: Subscription = pages.observeOn(eventScheduler) subscribe {
      x => x match {
        case Success(page) => editorpane.text = page
        case Failure(e) => status.text = "Page error " + e.getMessage
      }
    }
  }
}
