...
trait WikipediaApi {
  ...
  implicit class StringObservableOps(obs: Observable[String]) {
     ...
     */
    def sanitized: Observable[String] = obs.map { term => term.replaceAll(" ", "_") }
  ...
  implicit class ObservableOps[T](obs: Observable[T]) {
     ...
     */
    def recovered: Observable[Try[T]] = obs.map { t => Success(t) }.onErrorReturn { exception => Failure(exception) }
     ...
     */
    def timedOut(totalSec: Long): Observable[T] = obs.takeUntil(Observable.interval(Duration(totalSec, TimeUnit.SECONDS)))
